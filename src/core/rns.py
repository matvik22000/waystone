...
class AnnounceHandler:
    ...
    def received_announce(self, destination_hash, announced_identity: RNS.Identity, app_data):
        destination = RNS.prettyhexrep(destination_hash)
        logging.getLogger(self.aspect_filter).debug("received announce ident: %s, dst: %s, data: %s",
                                                    announced_identity, destination, app_data)

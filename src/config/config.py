...
class Config:
    def __post_init__(self):
        ...
        for variable, value in vars(type(self)).items():
            if not variable.startswith("__") and not self.__annotations__.get(variable) and isinstance(value, Field):
                suspicious_variables.append(variable)
        if suspicious_variables:
            raise Exception(f"{suspicious_variables} are specified as fields, bot haven't got types. "
        ...
        for variable, tp in self.__annotations__.items():
            ...
            try:
                ...
                raise Exception(f'{variable}: expected {tp}, found in env {fromEnv}, default {field.default}')

        if not_uppercase:
            getLogger().warning('All env variable names should be uppercase. %s were implicitly casted to uppercase',
                                not_uppercase)

        if not_presented:
            raise Exception(f'Required values are not presented in environ: {not_presented}')

        getLogger().info("Config loaded: %s", str(self))

    def __str__(self):
        return "Config(" + \
            ", ".join([f"{entry[0]}={str(entry[1])}" for entry in filter(lambda s: True, vars(self).items())]) + \
            ")"
